...
#  }
  ...
  obs.growth <- log(c(fit$N[-1], NA)) - log((fit$N))
  Nmax <- max(fit$N, na.rm = T)
  ...
  if (is.R) {
    ...
    plot(Nrange[is.finite(pred) & (pred < max(3 * abs(obs.growth), na.rm = T))], 
         pred[is.finite(pred) & (pred < max(3 * abs(obs.growth), na.rm = T))], 
         type = "n", ylab = "predicted/observed r", xlab = "N", 
         ylim = range(c(obs.growth, 
                        pred[is.finite(pred) & (pred < max(3 * abs(obs.growth), na.rm = T))]), 
                      na.rm = T), 
         sub = paste("s: ", round(fit$s, 3), "  (sig_e)^2: ", round(fit$sig2, 3)))
    ...
    if (length(sel) > 0) 
      text(fit$N[sel], obs.growth[sel], labels = year[sel], pos = 4, cex = 0.84)
    if (!is.null(lok)) title(lok)
    plot(year, log(fit$N), type = "n", 
         ylim = range(log(fit$N), na.rm = T) - c(0.1, -0.1), 
         ylab = "observed/predicted ln N")
    points(year, log(fit$N))
    lines (year, log(fit$N))
    points(year, fit$predicted, col = "red", pch = 16)
    plot(year, obs.growth, type = "n", 
         ylim = range(c(obs.growth, diff(fit$predicted)), na.rm = T), 
         xlim = range(year), 
         ylab = "observed/predicted r")
    points(year, obs.growth)
    lines (year, obs.growth)
    points(year, -0.5 * fit$sigd * exp(-log(fit$N)) + fit$s, 
           col = "red", pch = 16)
    abline(h = 0, lty = 2)
    ...
    if (prob < 0.05) abline(lmfit, lwd = 2, col = "red")
    ...
    acf(fit$residuals, na.action = na.pass, lag.max = 10, 
        main = "", sub = paste("whiteness p-val: ", round(port[3], 3)))
