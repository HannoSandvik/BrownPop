...
popmod <- function(pop.size, years=NULL, catch=NULL, sd=0.1, cov1=NULL, cov2=NULL, cov3=NULL, nboot=10000, nsim=0, name="", plot=TRUE, se=F, r=F, b1=F, b2=F, b3=F, mean1=F, mean2=F, mean3=F, bootout=TRUE, K=F){
  ...
  {test <- optim(par, lik, 
                 control=list(trace=F,maxit=12000,beta=0.1,gamma=1.2))
  return(test)}
  ...
  #    else
  #     {? <- file.remove(bootout)}}}
  if (is.null(years)) stop ("timepoints of data must be specified, if no NA's, try years=1:length(X)")
  if (is.null(sd)) stop ("demographic variance (sd) not specified, try sd=0")
  if (any(X > 30, na.rm=T))
  {cat("\nOBS: Jeg har en viss mistanke om at du har glemt " %+%
         "? log-transformere bestandstallene.\n\n")}
  if (any(X < 0, na.rm=T)) stop ("N kan ikke v?re mindre enn 1!")
  for (i in 1:ncol(X))
  {if (length(na.omit(X[,i])) < 6)
  {if (ncol(X) %=% 1)
  {stop ("For f? ?r med data!")}
    else
    {stop ("For f? ?r med data i kolonne " %+% "i" %+% "!")}}}
  ...
  {cat("\nOBS: ?rstallene er ikke sammenhengende. Er det meningen?\n\n")
  ...
  if ((length(Cov1) > 1) & !b1 & mean1)
  {Cov1 <- list(apply(cov1, 1, mean))}
  if ((length(Cov2) > 1) & !b2 & mean2)
  {Cov2 <- list(apply(cov2, 1, mean))}
  if ((length(Cov3) > 1) & !b3 & mean3)
  {Cov3 <- list(apply(cov3, 1, mean))}
  ...
  cov2[[i]][NAS] <- NA
  cov3[[i]][NAS] <- NA
  if(any(na.omit(cov1[[i]])!=0))
  {cov1[[i]] <- cov1[[i]]-mean(cov1[[i]],na.rm=T)}
  if(any(na.omit(cov2[[i]])!=0))
  {cov2[[i]] <- cov2[[i]]-mean(cov2[[i]],na.rm=T)}
  if(any(na.omit(cov3[[i]])!=0))
  {cov3[[i]] <- cov3[[i]]-mean(cov3[[i]],na.rm=T)}}
  ...
  est.k[i] <- est.r[i] - 
    ln(max(10, min(N[[i]],na.rm=T)/10, -coef(fit)[1]/coef(fit)[2]))}
  par[1:nk] <- est.k
  if (nr > 1)
  {par[nk + 1:nr] <- est.r}
  else
  ...
  predicted[[i]] <- c(NA, ww + my.est[i] -
                        ...
                        b1.est[i]*cov1[[i]] + b2.est[i]*cov2[[i]] + b3.est[i]*cov3[[i]] -
                        0.5*sd*exp(-ww))
  ...
  df.res <- data.frame("years"=years[[i]], "residual"=residual[[i]],
                       "demcomp"=demcomp[[i]], "predicted"=predicted[[i]])
  ...
  {b1 <- b1.est}
    ...
    {b1 <- rep(0, length(b1.est))}
    if (b2)
    {b2 <- b2.est}
    else
    {b2 <- rep(0, length(b2.est))}
    if (b3)
    {b3 <- b3.est}
    else
    {b3 <- rep(0, length(b3.est))}
    sim <- function(i)
  ...
  {cat("bootstrapping\n")
    ...
    rownames(bootres) <- c("min", "0.05%", "0.1%", "0.5%", "1%", "2.5%",
                           "5%", "25%", "mean", "75%", "95%", "97.5%", "99%", "99.5%", 
                           "99.9%", "99.95%", "max", "median")
    ...
    {bootres[1:17,j] <- quantile(boottab[,j], 
                                 c(0, 0.0005, 0.001, 0.005, 0.01, 0.025, 0.05, 0.25, 0.5, 
                                   0.75, 0.95, 0.975, 0.99, 0.995, 0.999, 0.9995, 1), na.rm=T)
  ...
  for (i in 1:npop)
  {
    ...
    portman.Q <- function (X, K) {
      ...
      Q <- n * (n + 2) * Q
      res <- list(chisq = signif(Q, 3), df = K, p.val = signif(1 - 
                                                                 pchisq(Q, K), 3))
      unlist(res)
    }
